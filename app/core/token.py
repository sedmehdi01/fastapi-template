...
from .common_schema import TokenType
...
async def add_token_to_redis(
    ...
    valid_tokens = await get_valid_tokens(redis_client, user_id, token_type)
    await redis_client.sadd(token_key, token)
    if not valid_tokens:
        await redis_client.expire(token_key, timedelta(minutes=expire_time))


async def get_valid_tokens(redis_client: Redis, user_id: str, token_type: TokenType):
    token_key = f"user:{user_id}:{token_type}"
